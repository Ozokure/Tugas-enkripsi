...
def encrypt_text(text):
    ...
    encrypted_text = ''.join(chr((ord(char) - 65 + 1) % 26 + 65) if char.isupper() else
                             chr((ord(char) - 97 + 1) % 26 + 97) if char.islower() else
                             char
                             for char in text)
    return encrypted_text

def decrypt_text(text):
    # Shift each character by -1 in the ASCII table to reverse the encryption
    decrypted_text = ''.join(chr((ord(char) - 65 - 1) % 26 + 65) if char.isupper() else
                             chr((ord(char) - 97 - 1) % 26 + 97) if char.islower() else
                             char
                             for char in text)
